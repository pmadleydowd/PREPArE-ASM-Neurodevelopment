preserve
	...
	merge 1:1 lopnr_ip using "P:\AED_ASD_VA\Dohad Data\asm2x_preg01_compressed.dta", gen(dx2)
	drop if dx2==2
...
egen max = rowmax(brivaracetam_pdr_preg2x_bm carbamazep_pdr_preg2x_bm eslikarbazep_pdr_preg2x_bm etosuximid_pdr_preg2x_bm felbamate_pdr_preg2x_bm fosphenytoin_pdr_preg2x_bm gabapentin_pdr_preg2x_bm klonazepam_pdr_preg2x_bm lacosamide_pdr_preg2x_bm lamotrigine_pdr_preg2x_bm levetiracetam_pdr_preg2x_bm oxcarbazep_pdr_preg2x_bm perampanel_pdr_preg2x_bm phenobarbital_pdr_preg2x_bm phenytoin_pdr_preg2x_bm pregabalin_pdr_preg2x_bm primidone_pdr_preg2x_bm retigabine_pdr_preg2x_bm rufinamid_pdr_preg2x_bm stiripentol_pdr_preg2x_bm sultiame_pdr_preg2x_bm tiagabine_acid_pdr_preg2x_bm topiramate_pdr_preg2x_bm valproic_acid_pdr_preg2x_bm vigabatrin_pdr_preg2x_bm zonisamide_pdr_preg2x_bm clobazam_pdr_preg2x_bm)

egen totaldx2 = rowtotal(brivaracetam_pdr_preg2x_bm carbamazep_pdr_preg2x_bm eslikarbazep_pdr_preg2x_bm etosuximid_pdr_preg2x_bm felbamate_pdr_preg2x_bm fosphenytoin_pdr_preg2x_bm gabapentin_pdr_preg2x_bm klonazepam_pdr_preg2x_bm lacosamide_pdr_preg2x_bm lamotrigine_pdr_preg2x_bm levetiracetam_pdr_preg2x_bm oxcarbazep_pdr_preg2x_bm perampanel_pdr_preg2x_bm phenobarbital_pdr_preg2x_bm phenytoin_pdr_preg2x_bm pregabalin_pdr_preg2x_bm primidone_pdr_preg2x_bm retigabine_pdr_preg2x_bm rufinamid_pdr_preg2x_bm stiripentol_pdr_preg2x_bm sultiame_pdr_preg2x_bm tiagabine_acid_pdr_preg2x_bm topiramate_pdr_preg2x_bm valproic_acid_pdr_preg2x_bm vigabatrin_pdr_preg2x_bm zonisamide_pdr_preg2x_bm clobazam_pdr_preg2x_bm)

generate super_exposure_dx2 = .
replace super_exposure_dx2 = 10 if n03_pdr_preg2x_bm==. | n03_pdr_preg2x_bm==0
replace super_exposure_dx2 = 0 if lamotrigine_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 1 if carbamazep_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 2 if gabapentin_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 3 if levetiracetam_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 4 if phenytoin_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 5 if pregabalin_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 6 if topiramate_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 7 if valproic_acid_pdr_preg2x_bm==1 & poly<=1
replace super_exposure_dx2 = 8 if  max==1 & super_exposure_dx2==.
replace super_exposure_dx2 = 9 if  totaldx2>1
replace super_exposure_dx2 = 11 if  super_exposure!=10 & super_exposure_dx2==.

label define super_exposure_dx2  ///
0 "Mono x2 Lamotrigine" ///
1 "Mono x2 Carbamazepine" ///
2 "Mono x2 Gabapentin" ///
3 "Mono x2 Levetiracetam" ///
4 "Mono x2 Phenytoin" ///
5 "Mono x2 Pregabalin" ///
6 "Mono x2 Topiramate" ///
7 "Mono x2 Valproic Acid" ///
8 "Mono x2 Other ASM" ///
9 "Polytherapy x2" ///
11 "x1 ASM" ///
10 "No ASM" 
label values super_exposure_dx2 super_exposure_dx2 


table super_exposure_dx2 outcome_ASD 
table super_exposure_dx2 outcome_ADHD
table super_exposure_dx2 outcome_ID

table super_exposure_dx2 outcome_NDD
...
*hist FU_ndd, by(outcome) xtitle("Age at censoring")
		  generate  flag_other_T1 = n03_pdr_trimester1_bm

		  foreach asm in carbamazep gabapentin lamotrigine levetiracetam phenytoin pregabalin topiramate valproic_acid {
		  replace flag_other_T1 = 2 if `asm'_pdr_trimester1_bm==1
		  }
	
	gen other_filter_T1 = flag_other_T1
replace other_filter_T1 = 0 if other_filter_T1==2

drop flag_other_T1

egen poly_T1 = rowtotal(lamotrigine_pdr_trimester1_bm carbamazep_pdr_trimester1_bm gabapentin_pdr_trimester1_bm levetiracetam_pdr_trimester1_bm phenytoin_pdr_trimester1_bm pregabalin_pdr_trimester1_bm topiramate_pdr_trimester1_bm valproic_acid_pdr_trimester1_bm other_filter_T1)

gen super_T1 = .

local num = 0
		  foreach asm in lamotrigine carbamazep gabapentin  levetiracetam phenytoin pregabalin topiramate valproic_acid {
		  replace super_T1 = `num' if `asm'_pdr_trimester1_bm==1
		  local num = `num'+1
		  }
	replace super_T1 = 8 if other_filter_T1==1 & super_T1==.
	replace super_T1 = 9 if poly_T1>1
	replace super_T1 = 10 if super_T1==.

	label define super_T1  ///
0 "Mono T1 Lamotrigine" ///
1 "Mono T1 Carbamazepine" ///
2 "Mono T1 Gabapentin" ///
3 "Mono T1 Levetiracetam" ///
4 "Mono T1 Phenytoin" ///
5 "Mono T1 Pregabalin" ///
6 "Mono T1 Topiramate" ///
7 "Mono T1 Valproic Acid" ///
8 "Mono T1 Other ASM" ///
9 "Polytherapy T1" ///
10 "No ASM T1" 
label values super_T1 super_T1 
...
rename super_T1 exposure

foreach var of varlist bcountry_bm_nm dispink5atbirth education addicted paritet seizure_visit sambo_atbirth_bm visits_1y_cat epilepsy depression_bm_preg  bipolar_bm_preg  other_psych_bm_preg  migraine_bm_preg  diabetes n05 a04 n06a pain sjukvarsregion male any_ndd_bm rounded_pregyear {
	
	quietly tab `var', gen(dum_`var')
}

quietly tab exposure, gen(exposure_dum)
drop if birthdate_ip<td(1jul2005)


 global covar age_atbirth_bm age_atbirth_bm_cube bcountry_bm_nm dum_dispink5atbirth1 dum_dispink5atbirth2 dum_dispink5atbirth3 dum_dispink5atbirth4 dum_education1 dum_education2 dum_addicted1 dum_paritet1 dum_paritet2 dum_paritet3 dum_paritet4 dum_seizure_visit1 dum_sambo_atbirth_bm1 dum_visits_1y_cat1 dum_visits_1y_cat2 dum_depression_bm_preg1 dum_bipolar_bm_preg1 dum_other_psych_bm_preg1 dum_n051 dum_a041 dum_n06a1 dum_sjukvarsregion1 dum_sjukvarsregion2 dum_sjukvarsregion3 dum_sjukvarsregion4 dum_sjukvarsregion5 dum_male1 dum_any_ndd_bm1 dum_diabetes1 dum_pain1 dum_migraine_bm_preg1 dum_epilepsy2 dum_rounded_pregyear1 dum_rounded_pregyear2 dum_rounded_pregyear3 dum_rounded_pregyear4 dum_rounded_pregyear5 dum_rounded_pregyear6 dum_rounded_pregyear7 dum_rounded_pregyear8
	...
	est store stpm2_`var'

	
	 }
	 
	 estout stpm2* using "C:\Users\vikahl\OneDrive - Karolinska Institutet\Skrivbordet\standsurv export\output_T1_oct2024.xls", keep(*exposure*)  cells("b(fmt(5)) se(fmt(5)) ci(fmt(5))") label replace
