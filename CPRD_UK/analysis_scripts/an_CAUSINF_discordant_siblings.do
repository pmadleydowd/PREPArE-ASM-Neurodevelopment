cap log close
log using "$Logdir\NDD study\LOG_an_CAUSINF_discordant_siblings.txt", text replace
********************************************************************************
* do file author:	Paul Madley-Dowd
* Date: 			24 May 2022
* Description : 	Perform discordant sibling analyses
********************************************************************************
...
foreach ndd in NDD ASD ID ADHD {
	...
	egen mean_`ndd' = mean(outcome_`ndd'), by(mumpatid) 
	gen disc_`ndd' = !inlist(mean_`ndd', 0, 1, .)
}

foreach ASM in carbamazepine gabapentin lamotrigine levetiracetam phenytoin pregabalin topiramate valproate other { 
	egen between_`ASM' = mean(flag_`ASM'_preg ), by(mumpatid)
}
...
postfile `memhold' str4 NDD NDD_n ASM_n ///
				   n_exp n_wNDD n_exp_wNDD ///
   				   n_exp_outdisc n_wNDD_outdisc n_exp_wNDD_outdisc ///
		  ...
		  using "$Datadir\NDD_study_PMD\Outputs\Causal_inference\NDD_causinf_discordant_siblings.dta", replace
...
foreach ndd in NDD ASD ID ADHD {
	...
	* unadjusted HR
	stpm2 mther_* between_*, df(4) scale(hazard) vce(cluster mumpatid) failconvlininit
	matrix unadj_`ndd'_table = r(table)
	
	* adjusted HR 
	stpm2 mther_* between_* $conflist, df(4) scale(hazard) vce(cluster mumpatid) failconvlininit
	...
	* store values 
	forvalues j = 0(1)10 {
		...
		count if monother_cat_preg == `j' & disc_`ndd' == 1
		local nexp = r(N)
		count if outcome_`ndd' == 1 & disc_`ndd' == 1
		local nout = r(N)
		count if monother_cat_preg == `j' & outcome_`ndd' == 1 & disc_`ndd' == 1
		local nexp_out = r(N)
	...
	* standardised survival function 
	...
	drop if timevar == . 
	gen NDD = "`ndd'"
	save "$Datadir\NDD_study_PMD\Outputs\Causal_inference\NDD_causinf_discsib_std_surv_`ndd'.dta", replace 
...
* 3 - Format output dataset for hazard ratios
********************************************************************************
use "$Datadir\NDD_study_PMD\Outputs\Causal_inference\NDD_causinf_discordant_siblings.dta", clear
...
sort NDD_n ASM_n
save "$Datadir\NDD_study_PMD\Outputs\Causal_inference\NDD_causinf_discordant_siblings.dta", replace
