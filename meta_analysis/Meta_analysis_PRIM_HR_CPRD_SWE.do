cap log close 
log using "$Logdir\NDD study\LOG_Meta_analysis_PRIM_HR_CPRD_SWE.txt", text replace
******************************************************************************
...
import excel "C:\Users\pm0233\University of Bristol\grp-PREPArE - Manuscripts\asm-ndd combining\Results 2023-11-16.xlsx", sheet("log(HR) log(SE-HR) log(CI-HR)") firstrow clear
drop *_logHRci95
...
append using "$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_HR_byASM.dta", 
drop if NDD == "NDD"
...
cap drop _merge
save "$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_metaready", replace
...
save  "$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_wgts.dta", replace
...
foreach NDD in ASD ADHD ID {
	...
	save "$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_metan_`NDD'.dta", replace
}
...
foreach ndd in "ASD" "ID" "ADHD" {
	...
	* collate data 
	use"$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_metaready", clear
	merge 1:1 ASM_n NDD_n country using "$Datadir\NDD_study_PMD\Outputs\Primary\NDD_prim_wgts.dta"
	foreach NDD in ASD ADHD ID {
...
graph export "$Graphdir\NDD_study_PMD\Prim_survival_analysis.png", replace width(2400) height(1600)
...
graph export "$Graphdir\NDD_study_PMD\Prim_survival_analysis_combined_only.png", replace width(2400) height(1600)
